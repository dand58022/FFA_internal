...
| `pdf-lib` + optional `@pdf-lib/fontkit` | M0 baseline for main-owned field writing and appearance generation; unmaintained since 2021 and no garbage collection on save, so replacement candidates are evaluated in M0 per the [stack ADR](decisions/2026-09-14-desktop-pdf-stack.md); fontkit only if needed for supported names |
...
Primary option: PDF.js canvas/text layer plus our own accessible text/checkbox overlays positioned at annotation rectangles. The inventory contains only text fields and independent checkboxes, so overlays cover every supported widget type and keep widget semantics in app code. Fallback within scope: PDF.js annotation-layer widgets with controlled event capture/patching, selected only if overlays fail precise zoom/scroll alignment or keyboard editing in M0. Do not rely on a Chromium built-in PDF iframe, whose input cannot reliably express app overrides.
...
Intake client pages 1-6 are a proposed subset. Build the client copy as a fresh document that receives only the permitted pages; never remove pages from a full copy, because the baseline library writes every loaded object and leaves removed content streams and field values in the file as unreferenced objects. Copying pages can lose AcroForm relationships; recreate the field tree for copied widgets and verify fields and widgets after export. An audience subset must remove excluded internal values, orphan fields, unused objects and reachable metadata, not merely hide a page. M0 must prove absence by decoding every object of the client output, including compressed object streams, and searching for page 7 field names, a synthetic fee-cell canary and page 7 content, before the finalizer can support this policy. Preserve consent page 1. Keep documents separate to avoid duplicate consent field names colliding in a merge.
...
New Outlook does not support the Outlook Object Model according to [Microsoft's feature comparison](https://support.microsoft.com/en-us/outlook/getstarted/feature-comparison-between-new-outlook-and-classic-outlook); it is not a substitute for the specified Classic Outlook target. New Windows and Microsoft 365 installations increasingly default to New Outlook, so the demo laptop and any pilot machine must be checked before assuming the COM path exists. Enterprise policies can also trigger restrictions/prompts; [Microsoft's security discussion](https://learn.microsoft.com/en-us/visualstudio/vsto/specific-security-considerations-for-office-solutions?view=visualstudio) documents Object Model Guard considerations.

**Documented alternative, not built in Stage 1:** Microsoft Graph can create a draft message in the signed-in user's mailbox and add file attachments to it, which works regardless of Outlook client variant. It requires Entra sign-in, application registration and online access, and it moves the draft boundary from a local COM call to a cloud API call. It is the candidate path if the target environment has only New Outlook or blocks scripts, and it is recorded in the [handoff ADR](decisions/2026-09-14-local-storage-and-handoff.md) so a later decision does not start from scratch.
