...
|---|---|---|
| Can PDF.js support immediate updates/direct edits without focus or state loss? | Prove both real forms and dense matrices; choose annotation-layer adapter or controlled overlays. Do not downgrade to static preview. | M0 exit / technical lead |
| Can pdf-lib output valid visible checkmarks, preserve field trees/signature areas and safely remove internal fee data? | Normalize appearances on copies, use exact `/1` states, test read-only fields and page-subset reserialization with independent reader/render. | M0/M7 / technical lead |
...
|---|---|---|
| Classic Outlook present/configured? PowerShell permitted? | Classic Outlook with a configured local profile; fixed helper. C# bridge is contingency if scripts blocked, not a policy bypass. | M1 early check and M8 / demo owner + IT |
