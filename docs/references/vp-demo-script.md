...
| 0:20-0:40 | Select both TWS forms, Client Profile - Sample and Product Application - Sample; Start | "Choose the forms needed for this case. The sample forms show how the same workflow can expand." | Four selected tabs, real/sample badges, split workspace |
...
| 2:30-3:00 | Verify Forms; click the missing quarterly-statements issue | "Before approval, the app checks the requirements we configured and takes me directly to the missing answer." | Red issue, approval disabled, page 2 exact pair highlighted |
...
| 3:40-4:20 | Prepare Client Email | "Outlook opens with the documents attached. The advisor reviews the draft and controls sending." | Normal Classic Outlook draft with recipient, subject, body and four client PDFs; no automatic send |
